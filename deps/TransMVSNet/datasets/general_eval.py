...
class MVSDataset(Dataset):
    ...
    def visualize_cam_grid(self, show=True):
        cam_paths = [os.path.join(self.datapath, 'scan1/cams/{:0>8}_cam.txt'.format(vid)) for vid in range(64)]
        extrinsics = torch.from_numpy(np.stack([self.read_cam_file(f, interval_scale=1.06)[1] for f in cam_paths]))
        ...
        for i, color in enumerate(["red", "green", "blue"]):
            ax.quiver(all_centers[:, 0, 0], all_centers[:, 1, 0], all_centers[:, 2, 0],
                      ...
                      edgecolor=color)
        for i, id in enumerate(range(64)):
    ...
    def visualize_item(self, i=0):
        ...
        self.visualize_cam_grid(show=False)
        ...
        for i, vid in enumerate(view_ids):
            ...
            intrinsics, extrinsics, depth_min, depth_interval = self.read_cam_file(proj_mat_filename, interval_scale=1.06)
            cam_center = -torch.from_numpy(extrinsics[:3, :3]).T @ torch.from_numpy(
                extrinsics[:3, -1:])
            ax = plt.gca()
            ax.scatter(*cam_center.flatten().unbind(dim=0), s=50, c="black" if i == 0 else "blue")
        plt.show()

if __name__ == "__main__":
    ds = MVSDataset("/is/rg/ncs/datasets/DTUTransMVSNet/dtu_test", ["scan1"], mode="test", nviews=5,
                    max_h=864, max_w=1152)
