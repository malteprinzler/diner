...
parser.add_argument('--mode', default='train', help='train or test',
                    choices=['train', 'test', 'profile', 'write_prediction'])
parser.add_argument('--model', default='mvsnet', help='select model')
parser.add_argument('--outdepthname', default='TransMVSNet', help='output depth name', type=str)
parser.add_argument('--maskoutput', action="store_true")
parser.add_argument('--device', default='cuda', help='select model')
parser.add_argument('--dataset', default='dtu_yao', help='select dataset')
parser.add_argument('--trainpath', help='train datapath')
parser.add_argument('--outpath', help='output dataset root path', default="")
parser.add_argument('--testpath', help='test datapath')
parser.add_argument('--trainlist', help='train list', default=None)
parser.add_argument('--testlist', help='test list', default=None)
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        print(
                            "Epoch {}/{}, Iter {}/{}, test loss = {:.3f}, depth loss = {:.3f}, entropy loss = {:.3f}, time = {:3f}".format(
...
@torch.no_grad()
def write_prediction(model, dataloaders, args):
    if is_distributed:
    ...
    for loader in dataloaders:
        for batch_idx, sample in tqdm.tqdm(enumerate(loader), total=len(loader)):
            ...
            scale_factor = 872. / 0.7 if args.dataset == "dtu_yao" else getattr(loader.dataset, "scale_factor", 1.)
            depth_est /= scale_factor
            conf = outputs["photometric_confidence"].clone()
            if args.maskoutput:
                depth_est[~mask.bool()] = 0
                conf[~mask.bool()] = 0
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    try:
        ...
        loss, depth_loss, entropy_loss, depth_entropy = model_loss(outputs, depth_gt_ms, mask_ms,
                                                                   dlossw=[float(e) for e in args.dlossw.split(",") if
                                                                           e])
...
if __name__ == '__main__':
    ...
    train_dataset = MVSDataset(args.trainpath, args.trainlist, args.mode, args.nviews, args.numdepth,
                               args.interval_scale)
    ...
    if is_distributed:
        ...
        TrainImgLoader = DataLoader(train_dataset, args.batch_size, sampler=train_sampler, num_workers=2,
                                    drop_last=True, pin_memory=args.pin_m)
        ...
        TrainImgLoader = DataLoader(train_dataset, args.batch_size, shuffle=True, num_workers=0, drop_last=False,
                                    pin_memory=args.pin_m)
    ...
    if args.mode == "train":
        ...
        profile()
    elif args.mode == "write_prediction":
        write_prediction(model, [TrainImgLoader, TestImgLoader], args)
    else:
